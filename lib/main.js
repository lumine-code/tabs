...
const TabTransferService = require("./tab-transfer-service.js");
...
module.exports = {
  activate() {
    ...
    this.tabBarViews = [];
    this.tabTransferService = new TabTransferService();
    this.subscriptions.add(this.tabTransferService);
    this.subscriptions.add(
      lumine.workspaceDrops.addProvider(
        new TabDropProvider(this.tabTransferService, {
          getTabBarForPane: (pane) => this.tabBarViews.find((view) => view.pane === pane),
        }),
        { priority: 100 },
      ),
    );
    ...
    this.subscriptions.add(
      lumine.commands.add("lumine-workspace", {
        "tabs:close-all-tabs": () => {
          return Promise.all(lumine.workspace.getPanes().map((pane) => pane.destroyItems()));
        },
      }),
    );
    ...
    Object.keys(paneContainers).forEach((location) => {
      ...
      this.subscriptions.add(
        container.observePanes((pane) => {
          if (pane.isDetached()) return;
          const tabBarView = new TabBarView(pane, location, this.tabTransferService);
