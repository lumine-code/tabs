module.exports = class TabDropProvider {
  ...
  async openRemoteItem(context, prepared) {
    ...
    const canRestoreTextEditorState =
      existedBeforeOpen &&
      typeof item.serializeViewState === "function" &&
      typeof item.restoreViewState === "function";
    const originalTextEditorState = canRestoreTextEditorState ? item.serializeViewState() : null;
    ...
    try {
      ...
      if (transferItem.textEditorState && typeof item.restoreViewState === "function") {
        item.restoreViewState(transferItem.textEditorState);
      }
    ...
    try {
      ...
      // The source has already committed, so activation failure cannot roll
      // the transfer back without deleting or overwriting its only copy.
      console.error(error);
    }
    try {
  ...
  async restoreRemoteItem(
    ...
    if (canRestoreTextEditorState) item.restoreViewState(originalTextEditorState);
