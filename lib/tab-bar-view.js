...
class TabBarView {
  ...
  onDragOver(event) {
    ...
    event.stopPropagation();
    event.dataTransfer.dropEffect = this.isFilePathEvent(event) ? "copy" : "move";
  ...
  onPaneDragOver(event) {
    ...
    const allowSplit = this.pane.getItems().length !== 0 && !this.isOnlyTabFromThisPane(event);
  ...
  onPaneDrop(event) {
    ...
    const isFileDrop = this.isFilePathEvent(event);
    const paths = isFileDrop ? this.getDraggedFilePaths(event) : null;
    if (!this.dropIsAllowed(event, this.location) || (paths && paths.length === 0)) {
      event.preventDefault();
      event.stopImmediatePropagation();
  ...
  isSupportedDropEvent(event) {
    return this.isLumineTabEvent(event) || this.isFilePathEvent(event);
  }
  ...
  dropIsAllowed(event, location) {
    if (this.isFilePathEvent(event)) return location === "center";
