...
class TabBarView {
  constructor(pane, location) {
    ...
    this.subscriptions.add(atom.window.onDidReceive("tabs:item-dropped", this.onDropOnOtherWindow));
  }
    ...
    if (fromWindowId === this.getWindowId()) {
      ...
      atom.workspace.open(droppedURI).then((item) => {
        ...
        if (!isNaN(fromWindowId)) {
          ...
          return atom.window.broadcast("tabs:item-dropped", {
