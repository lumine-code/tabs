...
const TabDragOutTracker = require("./tab-drag-out-tracker");

class TabBarView {
  constructor(pane, location, tabTransferService) {
    this.pane = pane;
    this.location = location;
    this.tabTransferService = tabTransferService;
    this.dragOutTracker = new TabDragOutTracker();
    ...
    this.subscriptions.add(
      lumine.commands.add(this.paneElement, {
        ...
        "tabs:detach-tab": {
          description: "Move this tab into a detached pane.",
          didDispatch: () => this.detachTab(this.getActiveTab()),
        },
      }),
    );
    ...
    this.element.addEventListener("dragleave", this.onTabBarDragLeave.bind(this));
    this.subscriptions.add(
      lumine.workspaceDrops.addTarget(
        this.element,
        {
          ...
          getIndex: (event) => this.getDropTargetIndex(event),
          onDragEnter: (context) => this.updateWorkspaceDropTarget(context),
          onDragOver: (context) => this.updateWorkspaceDropTarget(context),
  ...
  destroy() {
    ...
    this.unbindDragCancellation();
    this.dragOutTracker.reset();
  ...
  detachTab(tab = this.rightClickedTab || this.getActiveTab()) {
    if (this.location !== "center" || !tab?.item) return;
    return lumine.workspace.detachPaneItem(tab.item);
  }
  ...
  onDragStart(event) {
    let itemURI;
    let modifiedText;
    let allowedLocations = ["center", "left", "right", "bottom"];
    ...
    const item = this.draggedTab.item;
    if (item == null) {
      return;
    }

    const transferToken = this.tabTransferService.createSession(this.pane, item);
    this.dragToken = transferToken;
    if (this.location === "center") {
      this.dragOutTracker.start();
      this.bindDragCancellation();
    }
    ...
    if (typeof item.getAllowedLocations === "function") {
      const itemAllowedLocations = item.getAllowedLocations();
      if (itemAllowedLocations != null) allowedLocations = itemAllowedLocations;
    }
    ...
    if (itemURI != null) {
      ...
      if (process.platform === "darwin") {
        // see #69
        const externalURI = this.uriHasProtocol(itemURI) ? itemURI : `file://${itemURI}`;
    ...
    lumine.workspaceDrops.write(event.dataTransfer, {
      ...
      source: {
        ...
        paneId: this.pane.id,
        onlyItem: this.pane.getItems().length === 1,
      },
      items: [
        {
          type: "pane-item",
          uri: itemURI ?? null,
          ...(modifiedText == null ? {} : { modifiedText }),
        },
      ],
    });
  }
  ...
  onTabBarDragLeave(event) {
    // Placeholder cleanup belongs to the workspace drop manager, which
    // stabilizes ambiguous cross-window dragleave events for one frame. This
    // local listener only restores the tab widths used for mouse interaction.
    if (!event.currentTarget.contains(event.relatedTarget)) this.resetTabWidths();
  }

  onDrag(event) {
    if (this.location !== "center") return;
    this.dragOutTracker.update(
      event,
      lumine.workspace.getElement().getBoundingClientRect(),
  ...
  onDragEnd(event) {
    ...
    const detachPoint = this.dragOutTracker.getScreenPoint();
    const shouldDetach =
      this.location === "center" &&
      this.dragOutTracker.finish(
        event,
        lumine.workspace.getElement().getBoundingClientRect(),
        this.element.ownerDocument.defaultView,
      );
    ...
    if (shouldDetach) {
      void this.detachDraggedItem(item, token, detachPoint);
    } else if (dropEffect === "none") {
      this.tabTransferService.release(token, "tab drag cancelled");
    } else {
      this.tabTransferService.finishSession(token);
    }
  }

  async detachDraggedItem(item, token, point = null) {
    const pane = lumine.workspace.paneForItem(item);
    if (!pane || pane.isDetached()) {
      this.tabTransferService.release(token, "tab drag ended after its item moved");
      return false;
    }
    try {
      const options = point
        ? { bounds: { x: Math.round(point.x - 120), y: Math.round(point.y - 15) } }
        : undefined;
      await lumine.workspace.detachPaneItem(item, options);
      this.tabTransferService.release(token, "tab detached by drag-out");
      return true;
    } catch (error) {
      this.tabTransferService.release(token, "tab drag-out detach failed");
      lumine.notifications.addError("Could not detach tab", {
  ...
  bindDragCancellation() {
    this.unbindDragCancellation();
    this.dragWindow = this.element.ownerDocument.defaultView;
    this.dragCancellationHandler = (event) => {
      if (event.key === "Escape") this.dragOutTracker.cancel();
    };
    this.dragWindow.addEventListener("keydown", this.dragCancellationHandler, true);
  }

  unbindDragCancellation() {
    if (this.dragWindow && this.dragCancellationHandler) {
      this.dragWindow.removeEventListener("keydown", this.dragCancellationHandler, true);
    }
    this.dragWindow = null;
    this.dragCancellationHandler = null;
  }

  updateWorkspaceDropTarget(context) {
    const { offer, native, event } = context;
    const hasPaneItems = offer.kind === "pane-item";
    const hasSerializedFiles =
      (offer.kind === "paths" || offer.kind === "tree-entries") && offer.files;
    const hasNativeFiles = offer.native && (!native.known || native.hasFiles);
    if (!hasPaneItems && !hasSerializedFiles && !hasNativeFiles) {
      this.clearInsertionTarget();
      return;
    }
    this.updateInsertionTarget(event);
  }

  updateInsertionTarget(event) {
    let tab;
