...
const { CompositeDisposable, FileState } = require("lumine");
...
class TabBarView {
  constructor(pane, location) {
    ...
    this.boundOnPaneDragEnter = this.onPaneDragEnter.bind(this);
    this.boundOnPaneDragLeave = this.onPaneDragLeave.bind(this);
    this.boundOnPaneDragOver = this.onPaneDragOver.bind(this);
    this.boundOnPaneDrop = this.onPaneDrop.bind(this);
    this.paneElement.addEventListener("dragenter", this.boundOnPaneDragEnter, true);
    this.paneElement.addEventListener("dragleave", this.boundOnPaneDragLeave, true);
    this.paneElement.addEventListener("dragover", this.boundOnPaneDragOver, true);
    this.paneElement.addEventListener("drop", this.boundOnPaneDrop, true);
  ...
  destroy() {
    ...
    this.cancelPendingPaneDragLeave();
    this.resizeObserver.disconnect();
    this.subscriptions.dispose();
    this.paneElement.removeEventListener("dragenter", this.boundOnPaneDragEnter, true);
    this.paneElement.removeEventListener("dragleave", this.boundOnPaneDragLeave, true);
    this.paneElement.removeEventListener("dragover", this.boundOnPaneDragOver, true);
    this.paneElement.removeEventListener("drop", this.boundOnPaneDrop, true);
  ...
  closeSavedTabs() {
    return (() => {
      const result = [];
      for (let tab of this.getTabs()) {
        if ((tab.item.getFileState?.() ?? FileState.UNMODIFIED) === FileState.UNMODIFIED) {
          result.push(this.closeTab(tab));
        } else {
  ...
  onDragStart(event) {
    ...
    event.dataTransfer.setData(`lumine-tab-source-window-${this.getWindowId()}`, "true");
    event.dataTransfer.setData(`lumine-tab-source-pane-${this.pane.id}`, "true");
    if (this.pane.getItems().length === 1) {
      event.dataTransfer.setData("lumine-tab-source-pane-only-item", "true");
    }
    ...
    if (itemURI != null) {
      ...
      if (
        (item.getFileState?.() ?? FileState.UNMODIFIED) !== FileState.UNMODIFIED &&
        item.getText != null
      ) {
  ...
  onDragOver(event) {
    let tab;
    if (!this.isSupportedDropEvent(event)) {
      return;
    }
    if (!this.dropIsAllowed(event, this.location)) {
      return;
    }
    ...
    event.stopPropagation();
    event.dataTransfer.dropEffect = this.dropEffectForEvent(event);
    this.paneDropSplit = null;
    layout.disableView();

    if (this.isNativeFileEvent(event)) {
      const nativeDrag = this.getNativeDragSummary(event);
      if (nativeDrag.known && !nativeDrag.hasFiles) {
        this.removePlaceholder();
        this.lastDropTargetIndex = null;
        return;
      }
  ...
  onDrop(
    event,
    {
      ...
      removeEmptyPaneOnFailure = false,
    } = {},
  ) {
    if (this.isNativeFileEvent(event)) {
      const { filePaths, directoryPaths } = this.getNativeDroppedPaths(event);
      event.preventDefault();
      event.stopPropagation();
      layout.disableView();
      this.clearDropTarget();
      if (
        !this.dropIsAllowed(event, this.location) ||
        (filePaths.length === 0 && directoryPaths.length === 0)
      ) {
        return;
      }
      return this.openNativePaths({
        filePaths,
        directoryPaths,
        pane: toPane,
        index: toIndex,
        removeEmptyPaneOnFailure,
      });
    }

    if (this.isFilePathEvent(event)) {
      const paths = this.getDraggedFilePaths(event);
      event.preventDefault();
      event.stopPropagation();
      layout.disableView();
      this.clearDropTarget();
      if (!this.dropIsAllowed(event, this.location) || paths.length === 0) return;
    ...
    if (!this.isLumineTabEvent(event)) return;
    ...
    if (!this.itemIsAllowed(event, this.location)) {
      this.removeEmptyPaneAfterFailedDrop(toPane, removeEmptyPaneOnFailure);
      return;
    }

    if (fromWindowId === this.getWindowId()) {
      const fromPane = this.findPaneForDrop(event);
    ...
    const opening = lumine.workspace
      .open(droppedURI, { pane: toPane, activateItem: false, activatePane: false })
      .then((item) => {
        // An open can decline — an unreadable path, a full workspace center.
        // Never destroy the source item until a real target item exists.
        if (item == null) {
          this.removeEmptyPaneAfterFailedDrop(toPane, removeEmptyPaneOnFailure);
          return;
        }
        ...
        const openedIndex = openedPane.getItems().indexOf(item);
        this.moveItemBetweenPanes(openedPane, openedIndex, toPane, toIndex, item);
        if (hasUnsavedChanges && typeof item.setText === "function") {
    ...
    void lumine.window.focus();
    return opening;
  }

  onPaneDragOver(event) {
    if (event.target?.closest?.(".tab-bar") || !this.isSupportedDropEvent(event)) return;
    if (!this.dropIsAllowed(event, this.location)) {
    ...
    const itemViews = this.paneElement.querySelector(":scope > .item-views");
    if (!itemViews) return;
    ...
    const nativeDrag = this.isNativeFileEvent(event) ? this.getNativeDragSummary(event) : null;
    const allowSplit =
      this.pane.getItems().length !== 0 &&
      !this.isOnlyTabFromThisPane(event) &&
      (!nativeDrag || !nativeDrag.known || nativeDrag.hasFiles);
    const coords = allowSplit ? [event.clientX, event.clientY] : undefined;
    this.paneDropSplit = layout.updateView(itemViews, coords);
  }

  onPaneDrop(event) {
    if (event.target?.closest?.(".tab-bar") || !this.isSupportedDropEvent(event)) return;

    const isInternalFileDrop = this.isFilePathEvent(event);
    const isNativeDrop = this.isNativeFileEvent(event);
    const internalPaths = isInternalFileDrop ? this.getDraggedFilePaths(event) : null;
    const nativePaths = isNativeDrop
      ? this.getNativeDroppedPaths(event)
      : { filePaths: [], directoryPaths: [] };
    const hasPaths =
      (internalPaths?.length ?? 0) > 0 ||
      nativePaths.filePaths.length > 0 ||
      nativePaths.directoryPaths.length > 0;
    if (
      !this.dropIsAllowed(event, this.location) ||
      ((isInternalFileDrop || isNativeDrop) && !hasPaths)
    ) {
      event.preventDefault();
      event.stopImmediatePropagation();
      this.clearPaneDropTarget();
      return;
    }
    ...
    let split =
      this.pane.getItems().length === 0 || this.isOnlyTabFromThisPane(event)
        ? undefined
        : this.paneDropSplit;
    if (isNativeDrop && nativePaths.filePaths.length === 0) split = undefined;
    this.clearPaneDropTarget();

    if (!isInternalFileDrop && !isNativeDrop) {
      const fromWindowId = parseInt(event.dataTransfer.getData("from-window-id"));
      const fromPane = fromWindowId === this.getWindowId() ? this.findPaneForDrop(event) : null;
      if (fromPane === this.pane && split == null) return;
    }

    const toPane = layout.resolvePane(this.pane, split);
    const toIndex = toPane.getActiveItemIndex() + 1;
    const removeEmptyPaneOnFailure = toPane !== this.pane;
    ...
    if (isInternalFileDrop) {
      return this.openPaths(internalPaths, {
        pane: toPane,
        index: toIndex,
        removeEmptyPaneOnFailure,
      });
    ...
    return this.onDrop(event, { toPane, toIndex, removeEmptyPaneOnFailure });
  }

  // Show a hidden tab bar while a supported item is being dragged through its pane.
  onPaneDragEnter(event) {
    if (!this.isSupportedDropEvent(event) || !this.dropIsAllowed(event, this.location)) return;
    this.cancelPendingPaneDragLeave();
    event.preventDefault();
    event.dataTransfer.dropEffect = this.dropEffectForEvent(event);
    if (this.isNativeFileEvent(event)) {
      const nativeDrag = this.getNativeDragSummary(event);
      if (nativeDrag.known && !nativeDrag.hasFiles) return;
    }
    if (this.pane.getItems().length > 1 || lumine.config.get("tabs.alwaysShowTabBar")) return;
    return this.element.classList.remove("hidden");
  }
  ...
  onPaneDragLeave(event) {
    ...
    event.preventDefault();
    event.dataTransfer.dropEffect = this.dropEffectForEvent(event);
    if (this.paneElement.contains(event.relatedTarget)) return;

    if (event.relatedTarget == null) {
      this.cancelPendingPaneDragLeave();
      this.paneDragLeaveFrame = requestAnimationFrame(() => {
        this.paneDragLeaveFrame = null;
        this.completePaneDragLeave();
      });
      return;
    }

    return this.completePaneDragLeave();
  }
  ...
  onMouseDown(event) {
    ...
    const isRightClick = event.button === 2 || (event.button === 0 && event.ctrlKey === true);

    if (isRightClick) {
      // `rightClickedTab` is what every target-relative command resolves
      // against, so a right-click on the bar's empty space has to clear it
      // rather than leave the tab clicked before it standing in: that menu is
      // about the pane, and a stale target would act on a tab nobody pointed
      // at.
      this.setRightClickedTab(tab);
    }
  ...
  async openPaths(paths, { pane, index, removeEmptyPaneOnFailure = false }) {
    const openedItems = [];
    let nextIndex = index;
    ...
    for (const filePath of paths) {
      ...
      const fromPane = lumine.workspace.paneForItem(item);
      if (!fromPane) continue;
      const fromIndex = fromPane.getItems().indexOf(item);
      const itemIndex = this.moveItemWithoutActivation(fromPane, fromIndex, pane, nextIndex, item);
      nextIndex = itemIndex + 1;
      openedItems.push(item);
    }

    const activeItem = openedItems[openedItems.length - 1];
    if (!activeItem) {
      this.removeEmptyPaneAfterFailedDrop(pane, removeEmptyPaneOnFailure);
      return [];
    }
  ...
  openNativePaths({ filePaths, directoryPaths, pane, index, removeEmptyPaneOnFailure = false }) {
    if (directoryPaths.length > 0) {
      pane.activate();
      lumine.application.openWindow({ pathsToOpen: directoryPaths, here: true });
    }
    if (filePaths.length === 0) return;
    return this.openPaths(filePaths, { pane, index, removeEmptyPaneOnFailure });
  }

  moveItemWithoutActivation(fromPane, fromIndex, toPane, toIndex, item) {
    if (toIndex == null) toIndex = toPane.getActiveItemIndex() + 1;
    if (toPane === fromPane) {
      if (fromIndex < toIndex) toIndex--;
      if (fromIndex !== toIndex) toPane.moveItem(item, toIndex);
    } else {
      try {
    ...
    return toPane.getItems().indexOf(item);
  }

  moveItemBetweenPanes(fromPane, fromIndex, toPane, toIndex, item) {
    this.moveItemWithoutActivation(fromPane, fromIndex, toPane, toIndex, item);
    toPane.activateItem(item);
    return toPane.activate();
  }

  removeEmptyPaneAfterFailedDrop(pane, remove) {
    if (
      remove &&
      !pane.isDestroyed() &&
      pane.getItems().length === 0 &&
      pane.getContainer().getPanes().length > 1
    ) {
      return pane.destroy();
    }
  }
  ...
  clearPaneDropTarget() {
    this.cancelPendingPaneDragLeave();
    this.paneDropSplit = null;
    return layout.disableView();
  }

  cancelPendingPaneDragLeave() {
    if (this.paneDragLeaveFrame == null) return;
    cancelAnimationFrame(this.paneDragLeaveFrame);
    this.paneDragLeaveFrame = null;
  }

  completePaneDragLeave() {
    this.clearPaneDropTarget();
  ...
  findPaneForDrop(event) {
    ...
    if (fromPane?.id === fromPaneId) return fromPane;

    return Array.from(document.querySelectorAll("lumine-pane"))
      .map((paneElement) => paneElement.model)
  ...
  isLumineTabEvent(event) {
    return this.dataTransferHasType(event, "lumine-tab-event");
  }

  isFilePathEvent(event) {
    return this.dataTransferHasType(event, FILE_PATHS_TYPE);
  }

  isNativeFileEvent(event) {
  ...
  isSupportedDropEvent(event) {
    return (
      this.isLumineTabEvent(event) || this.isFilePathEvent(event) || this.isNativeFileEvent(event)
    );
  }

  dataTransferHasType(event, type) {
    for (const item of event.dataTransfer.items) {
  ...
  getDraggedFilePaths(event) {
    try {
      const paths = JSON.parse(event.dataTransfer.getData(FILE_PATHS_TYPE));
  ...
  getNativeDragSummary(event) {
    ...
    for (const item of event.dataTransfer.items) {
      ...
      try {
        ...
        // Protected drag data may withhold the entry until drop.
      }
      if (!entry) {
        known = false;
    ...
    return { known, hasFiles, hasDirectories };
  }

  getNativeDroppedPaths(event) {
    const filePaths = [];
    ...
    for (const file of Array.from(event.dataTransfer.files ?? [])) {
      ...
      try {
        ...
        // Spec fakes and older call sites may still provide a path property.
      }
      ...
      try {
        ...
        // The path may disappear between dragenter and drop; opening it as a
        // file preserves the editor's existing missing-file behavior.
      }
      if (stats?.isDirectory()) directoryPaths.push(filePath);
  ...
  dropEffectForEvent(event) {
    return this.isLumineTabEvent(event) ? "move" : "copy";
  }

  dropIsAllowed(event, location) {
    if (this.isFilePathEvent(event) || this.isNativeFileEvent(event)) {
      return location === "center";
    }
    return this.itemIsAllowed(event, location);
  }

  isOnlyTabFromThisPane(event) {
    if (!this.isLumineTabEvent(event)) return false;
    return (
      this.dataTransferHasType(event, "lumine-tab-source-pane-only-item") &&
      this.dataTransferHasType(event, `lumine-tab-source-window-${this.getWindowId()}`) &&
      this.dataTransferHasType(event, `lumine-tab-source-pane-${this.pane.id}`)
    );
  }
